...
Meteor.methods({
    ...
    addUserToCompetition: function (users, competitionId) {
        if(!Competitions.findOne({_id:competitionId})) {
            throw new Meteor.Error('invalid-competitio', 'You must select a valid competition');
        }

        Competitions.update({_id: competitionId}, {$addToSet: {users: {$each: users}}})
